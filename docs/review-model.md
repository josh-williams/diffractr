...
The diff gutter offers a comment button on hover. Clicking it opens an editor below that line; dragging it across lines creates a same-side range comment. The editor stays inline while saving, including errors and the explicit summary fallback. Only one new comment editor is active at a time.

GitHub is authoritative for pending review comments and the review summary. Saving a comment creates or reuses a pending review; draft edits/deletions are restricted to that user’s pending review. Published comments from all authors are loaded too, and the signed-in user can edit their own published comments. All review comments are paginated, including comments outside the captured source. Comments with an original or current revision matching the snapshot and an explicit side are projected inline. Pending comment coordinates are read from GitHub review threads because the review-comments REST endpoint omits them. Other drafts remain editable in Finish review.
...
Comment bodies and review summaries contain no tracking markers. The running server tracks operation IDs in memory to avoid repeating saves within that session. A lost comment response can be reconciled against newly returned comment IDs and matching source coordinates/body; an unconfirmed write is not repeated automatically. This retry history does not survive a server restart, so check GitHub before resending an uncertain save after reopening. Legacy markers are hidden when reading and removed when the body is edited. Submission addresses a specific pending review ID and checks its summary and comments against what the user saw. It never creates a second review as an automatic retry. Summary/comment edits reject observed external changes. These are preflight checks, not atomic locks against concurrent edits on GitHub or another local server.

The browser retains recovery copies of unsaved comment, edit, and summary text within its current origin. Saved drafts are not mirrored to a local feedback artifact. Remote drafts refresh on window focus and on demand. A review submitted or removed elsewhere is reflected on refresh. Reply creation, thread resolution, and automatic cross-revision migration remain outside this workflow.
